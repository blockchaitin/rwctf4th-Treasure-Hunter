...
library SMT{
    ...
    function smt_merge(bytes32 lhs, bytes32 rhs) internal pure returns (bytes32){
        if(lhs == 0){
            return rhs;
        }else if(rhs == 0){
            return lhs;
    ...
    function black_white_list_verify(bytes32[] memory _proofs,uint160 _bits,address _target,bytes32 _expectedRoot,bool is_white_list)internal pure returns (bool){
        if(is_white_list){
            smt_leaf memory leaf = smt_leaf({
                key : _target,
                value : 1
            });
    ...
    function smt_verify(bytes32[] memory _proofs,uint160 _bits,smt_leaf memory _leaf,bytes32 _expectedRoot) internal pure returns (bool){
        return (smt_calculate_root(_proofs,_bits,_leaf) == _expectedRoot);
    }

    function smt_update(bytes32[] memory _proofs,uint160 _bits,smt_leaf memory _nextleaf,smt_leaf memory _prevleaf,bytes32 _prevroot)internal pure returns (bytes32){
        require(
    ...
    function smt_calculate_root(bytes32[] memory _proofs,uint160 _bits,smt_leaf memory _leaf) internal pure returns (bytes32) {
